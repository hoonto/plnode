 ...
 * plnode_type.cc
 *
 * Originally: plv8_type.cc : Postgres from/to v8 data converters.
 *
...
static Datum ToScalarDatum(Handle<v8::Value> value, bool *isnull, plnode_type *type);
static Datum ToArrayDatum(Handle<v8::Value> value, bool *isnull, plnode_type *type);
static Datum ToRecordDatum(Handle<v8::Value> value, bool *isnull, plnode_type *type);
static Local<v8::Value> ToScalarValue(Datum datum, bool isnull, plnode_type *type);
static Local<v8::Value> ToArrayValue(Datum datum, bool isnull, plnode_type *type);
static Local<v8::Value> ToRecordValue(Datum datum, bool isnull, plnode_type *type);
...
void
plnode_fill_type(plnode_type *type, Oid typid, MemoryContext mcxt)
{
	bool    ispreferred;
		...
		if (HeapTupleIsValid(tp))
		{
						...
						"plnode_int4array") == 0)
			{
				type->ext_array = kExternalIntArray;
						...
						"plnode_float4array") == 0)
			{
				type->ext_array = kExternalFloatArray;
			}
			else if (strcmp(NameStr(typtup->typname),
						"plnode_float8array") == 0)
			{
...
Datum
ToDatum(Handle<v8::Value> value, bool *isnull, plnode_type *type)
{
	if (type->category == TYPCATEGORY_ARRAY)
...
static Datum
ToScalarDatum(Handle<v8::Value> value, bool *isnull, plnode_type *type)
{
...
static Datum
ToArrayDatum(Handle<v8::Value> value, bool *isnull, plnode_type *type)
{
	int			length;
	Datum	   *values;
	bool	   *nulls;
...
static Datum
ToRecordDatum(Handle<v8::Value> value, bool *isnull, plnode_type *type)
{
	Datum		result;
	TupleDesc	tupdesc;
...
Local<v8::Value>
ToValue(Datum datum, bool isnull, plnode_type *type)
{
	if (isnull)
...
ToScalarValue(Datum datum, bool isnull, plnode_type *type)
{
	switch (type->typid)
	{
	case OIDOID:
...
static Local<v8::Value>
ToArrayValue(Datum datum, bool isnull, plnode_type *type)
{
	Datum	   *values;
	bool	   *nulls;
	int			nelems;
...
static Local<v8::Value>
ToRecordValue(Datum datum, bool isnull, plnode_type *type)
{
...
Local<String>
ToString(Datum value, plnode_type *type)
{
