/*-------------------------------------------------------------------------
 *
 * plnode.cc : PL/v8 handler routines.
 * originally plv8.cc : PL/v8 handler routines.
 *
...
PG_FUNCTION_INFO_V1(plnode_call_handler);
PG_FUNCTION_INFO_V1(plnode_call_validator);
...
Datum	plnode_call_handler(PG_FUNCTION_ARGS) throw();
Datum	plnode_call_validator(PG_FUNCTION_ARGS) throw();
...
PG_FUNCTION_INFO_V1(plnode_inline_handler);
PG_FUNCTION_INFO_V1(plcoffee_inline_handler);
PG_FUNCTION_INFO_V1(plls_inline_handler);
Datum	plnode_inline_handler(PG_FUNCTION_ARGS) throw();
...
typedef struct plnode_proc_cache
{
	Oid						fn_oid;
 ...
 * lifetime is same as plnode_proc, but they are not palloc'ed memory,
 * so we need to clear them at the end of transaction.
 */
typedef struct plnode_exec_env
{
	Persistent<Object>		recv;
	Persistent<Context>		context;
	struct plnode_exec_env   *next;
} plnode_exec_env;

/*
 * We cannot cache plnode_type inter executions because it has FmgrInfo fields.
 * So, we cache rettype and argtype in fn_extra only during one execution.
 */
typedef struct plnode_proc
{
	plnode_proc_cache		   *cache;
	plnode_exec_env		   *xenv;
	TypeFuncClass			functypclass;			/* For SRF */
	plnode_type				rettype;
	plnode_type				argtypes[FUNC_MAX_ARGS];
} plnode_proc;
 ...
 */
typedef struct plnode_context
{
	Persistent<Context>		context;
	Oid						user_id;
} plnode_context;

static HTAB *plnode_proc_cache_hash = NULL;

static plnode_exec_env		   *exec_env_head = NULL;
 ...
 */
static plnode_proc *plnode_get_proc(Oid fn_oid, FunctionCallInfo fcinfo,
		bool validate, char ***argnames) throw();
static void plnode_xact_cb(XactEvent event, void *arg);

/*
 * CamelCaseFunctions are C++ functions.
 * They could raise errors with C++ throw statements, or never throw exceptions.
 */
static plnode_exec_env *CreateExecEnv(Handle<Function> script);
static plnode_proc *Compile(Oid fn_oid, FunctionCallInfo fcinfo,
...
static Datum CallFunction(PG_FUNCTION_ARGS, plnode_exec_env *xenv,
		int nargs, plnode_type argtypes[], plnode_type *rettype);
static Datum CallSRFunction(PG_FUNCTION_ARGS, plnode_exec_env *xenv,
		int nargs, plnode_type argtypes[], plnode_type *rettype);
static Datum CallTrigger(PG_FUNCTION_ARGS, plnode_exec_env *xenv);
static Persistent<Context> GetGlobalContext();
static Persistent<Context> GetNodeContext();
static Persistent<ObjectTemplate> GetGlobalObjectTemplate();

/* A GUC to specify a custom start up function to call */
static char *plnode_start_proc = NULL;

/* A GUC to specify the remote debugger port */
static int plnode_debugger_port;
/*
 * We use vector instead of hash since the size of this array
 * is expected to be short in most cases.
 */
static std::vector<plnode_context *> ContextVector;
...
_PG_init(void)
{
	...
	hash_ctl.entrysize = sizeof(plnode_proc_cache);
	hash_ctl.hash = oid_hash;
	plnode_proc_cache_hash = hash_create("PLv8 Procedures", 32,
									   &hash_ctl, HASH_ELEM | HASH_FUNCTION);

	DefineCustomStringVariable("plnode.start_proc",
	...
	DefineCustomIntVariable("plnode.debugger_port",
	...
	RegisterXactCallback(plnode_xact_cb, NULL);

	EmitWarningsOnPlaceholders("plnode");
}

static void
plnode_xact_cb(XactEvent event, void *arg)
{
	plnode_exec_env	   *env = exec_env_head;
...
static inline plnode_exec_env *
plnode_new_exec_env()
{
	plnode_exec_env	   *xenv = (plnode_exec_env *)
		MemoryContextAllocZero(TopTransactionContext, sizeof(plnode_exec_env));
...
common_pl_call_handler(PG_FUNCTION_ARGS, Dialect dialect) throw()
{
	...
	try
	{
		...
		{
			plnode_proc	   *proc = Compile(fn_oid, fcinfo,
										   false, is_trigger, dialect);
			proc->xenv = CreateExecEnv(proc->cache->function);
			fcinfo->flinfo->fn_extra = proc;
		}

		plnode_proc *proc = (plnode_proc *) fcinfo->flinfo->fn_extra;
		plnode_proc_cache *cache = proc->cache;
...
Datum
plnode_call_handler(PG_FUNCTION_ARGS) throw()
{
...
common_pl_inline_handler(PG_FUNCTION_ARGS, Dialect dialect) throw()
{
	...
	try
	{
        ...
        //MLM: Get NOde Context instead:
		Handle<Context>	global_context = GetNodeContext();
		...
		plnode_exec_env	   *xenv = CreateExecEnv(function);
...
Datum
plnode_inline_handler(PG_FUNCTION_ARGS) throw()
{
...
static Datum
CallSRFunction(PG_FUNCTION_ARGS, plnode_exec_env *xenv,
	int nargs, plnode_type argtypes[], plnode_type *rettype)
{
	plnode_proc		   *proc = (plnode_proc *) fcinfo->flinfo->fn_extra;
	TupleDesc			tupdesc;
...
static Datum
CallTrigger(PG_FUNCTION_ARGS, plnode_exec_env *xenv)
{
...
common_pl_call_validator(PG_FUNCTION_ARGS, Dialect dialect) throw()
{
	...
	try
	{
		...
		plnode_proc	   *proc = Compile(fn_oid, NULL,
...
Datum
plnode_call_validator(PG_FUNCTION_ARGS) throw()
{
...
static plnode_proc *
plnode_get_proc(Oid fn_oid, FunctionCallInfo fcinfo, bool validate, char ***argnames) throw()
{
	HeapTuple			procTup;
	plnode_proc_cache	   *cache;
	bool				found;
	...
	cache = (plnode_proc_cache *)
		hash_search(plnode_proc_cache_hash,&fn_oid, HASH_ENTER, &found);
	...
	plnode_proc *proc = (plnode_proc *) MemoryContextAllocZero(mcxt,
		offsetof(plnode_proc, argtypes) + sizeof(plnode_type) * cache->nargs);
	...
	for (int i = 0; i < cache->nargs; i++)
	{
		...
		plnode_fill_type(&proc->argtypes[i], argtype, mcxt);
	}
	...
	plnode_fill_type(&proc->rettype, rettype, mcxt);

	return proc;
}

static plnode_exec_env *
		...
		Dialect dialect)
{
	...
	PG_TRY();
	{
		proc = plnode_get_proc(fn_oid, fcinfo, validate, &argnames);
	}
	PG_CATCH();
	{
		throw pg_error();
	...
	if (cache->function.IsEmpty())
	{
		 ...
		 * Compile is called recursively and plnode_get_proc tries to refresh
...
find_js_function(Oid fn_oid)
{
	...
	Form_pg_proc	proc;
	Oid				prolang;
	NameData		langnames[] = { {"plnode"}, {"plcoffee"}, {"plls"} };
	int				langno;
...
//MLM: Start and get the Node.js context.
...
GetNodeContext()
{
	...
	{
		HandleScope				handle_scope;
		Handle<ObjectTemplate>	globaltemplate = GetGlobalObjectTemplate();
...
GetGlobalContext()
{
	...
	if (global_context.IsEmpty())
	{
		...
		my_context = (plnode_context *) MemoryContextAlloc(TopMemoryContext,
														 sizeof(plnode_context));
		my_context->context = global_context;
		my_context->user_id = user_id;
...
GetGlobalObjectTemplate()
{
	...
	if (global.IsEmpty())
	{
		...
		Handle<ObjectTemplate>	plnode = ObjectTemplate::New();
...
/*
 * Accessor to plnode_type stored in fcinfo.
 */
plnode_type *
get_plnode_type(PG_FUNCTION_ARGS, int argno)
{
	plnode_proc *proc = (plnode_proc *) fcinfo->flinfo->fn_extra;
