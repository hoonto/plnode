 ...
 * plnode_param.cc
 *
 * Originally: plv8_param.cc : PL/v8 parameter handling.
 *
...
static Node *plnode_variable_paramref_hook(ParseState *pstate, ParamRef *pref);
static Node *plnode_variable_coerce_param_hook(ParseState *pstate, Param *param,
						   Oid targetTypeId, int32 targetTypeMod,
						   int location);

void
plnode_variable_param_setup(ParseState *pstate, void *arg)
{
	plnode_param_state *parstate = (plnode_param_state *) arg;

	pstate->p_ref_hook_state = (void *) parstate;
	pstate->p_paramref_hook = plnode_variable_paramref_hook;
	pstate->p_coerce_param_hook = plnode_variable_coerce_param_hook;
}

static Node *
plnode_variable_paramref_hook(ParseState *pstate, ParamRef *pref)
{
	plnode_param_state *parstate = (plnode_param_state *) pstate->p_ref_hook_state;
...
static Node *
plnode_variable_coerce_param_hook(ParseState *pstate, Param *param,
							   Oid targetTypeId, int32 targetTypeMod,
							   int location)
{
	if (param->paramkind == PARAM_EXTERN && param->paramtype == UNKNOWNOID)
	{
		...
		plnode_param_state *parstate =
			(plnode_param_state *) pstate->p_ref_hook_state;
...
ParamListInfo
plnode_setup_variable_paramlist(plnode_param_state *parstate,
