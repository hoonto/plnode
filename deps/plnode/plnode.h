...
/* numbers for plnode object internal field */
/* Converter for SRF */
...
private:
	...
	std::vector< plnode_type >				m_coltypes;
	bool									m_is_scalar;
 ...
 * in plnode object for API to use it.  It would be possible to do it
...
private:
	...
	v8::Handle<v8::Object>	m_plnodeobj;
...
public:
						...
						FunctionCallInfo fcinfo)
	{
		...
		{
			m_plnodeobj = v8::Handle<v8::Object>::Cast(
					context->Global()->Get(v8::String::NewSymbol("plnode")));
			if (m_plnodeobj.IsEmpty())
				throw js_error("plnode object not found");
			/* Stash the current item, just in case of nested call */
			m_prev_fcinfo = m_plnodeobj->GetInternalField(PLV8_INTNL_FCINFO);
			m_plnodeobj->SetInternalField(PLV8_INTNL_FCINFO,
	...
	~WindowFunctionSupport()
	{
		...
		if (WindowObjectIsValid(m_winobj))
		{
			m_plnodeobj->SetInternalField(PLV8_INTNL_FCINFO, m_prev_fcinfo);
		}
	}
};
...
public:
			   ...
			   Converter *conv, Tuplestorestate *tupstore)
	{
		...
		if (m_plnodeobj.IsEmpty())
			throw js_error("plnode object not found");
		m_prev_conv = m_plnodeobj->GetInternalField(PLV8_INTNL_CONV);
		m_prev_tupstore = m_plnodeobj->GetInternalField(PLV8_INTNL_TUPSTORE);
		m_plnodeobj->SetInternalField(PLV8_INTNL_CONV,
									v8::External::New(conv));
		m_plnodeobj->SetInternalField(PLV8_INTNL_TUPSTORE,
									v8::External::New(tupstore));
	}
	~SRFSupport()
	{
		/* Restore the previous items. */
		m_plnodeobj->SetInternalField(PLV8_INTNL_CONV, m_prev_conv);
		m_plnodeobj->SetInternalField(PLV8_INTNL_TUPSTORE, m_prev_tupstore);
	}
};
...
extern plnode_type *get_plnode_type(PG_FUNCTION_ARGS, int argno);

// plnode_type.cc
extern void plnode_fill_type(plnode_type *type, Oid typid, MemoryContext mcxt = NULL);
extern Oid inferred_datum_type(v8::Handle<v8::Value> value);
extern Datum ToDatum(v8::Handle<v8::Value> value, bool *isnull, plnode_type *type);
extern v8::Local<v8::Value> ToValue(Datum datum, bool isnull, plnode_type *type);
extern v8::Local<v8::String> ToString(Datum value, plnode_type *type);
...
extern void SetupPlv8Functions(v8::Handle<v8::ObjectTemplate> plnode);
